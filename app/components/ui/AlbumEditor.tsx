...
export default function AlbumEditor({ layout }: { layout: PlantillaLayout }) {
  ...
  // Requiere sesión Supabase REAL (no demo), igual que el plan Minimal: sin eso no hay a quién
  // asociar el archivo subido ni un pedidoId real, así que en vez de degradar a una descarga local
  // se manda a iniciar sesión — el botón siempre debería terminar en un link de WhatsApp, no en una
  // descarga silenciosa.
  const onSend = async () => {
    if (!user) {
    ...
    try {
      ...
      try {
        ...
        console.warn('No se pudo registrar el pedido en Supabase; se manda el mensaje sin link.', err);
        setSendError('No pudimos subir tu álbum — te escribimos igual, cuéntanos por WhatsApp y lo resolvemos.');
      }
        ...
        <div style={{ maxWidth: 1240, margin: '0 auto', padding: '0 24px', height: 60, display: 'flex', alignItems: 'center', gap: 20 }} className="editor-topbar">
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            <button onClick={onSend} disabled={sending} style={{
              ...
              {sending ? 'GENERANDO...' : user ? 'ENVIAR A WHATSAPP' : 'INICIAR SESIÓN'}
            </button>
          </div>
        </div>
      </header>
