...
export default function PasosSection() {
  return (
    <section style={{ background: 'var(--marron)', padding: '80px 32px', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', top: -40, left: -40, fontSize: 120, color: 'rgba(245,247,246,0.06)' }}>✳</div>
      <div style={{ maxWidth: 1080, margin: '0 auto' }}>
        <p style={{ fontStyle: 'italic', fontSize: 24, color: 'var(--coral-suave)', textAlign: 'center', margin: '0 0 8px' }}>
          sin complicaciones
        </p>
        <h2 style={{ fontWeight: 800, fontSize: 'clamp(1.9rem,3vw,2.5rem)', color: 'var(--crema)', textAlign: 'center', margin: '0 0 56px' }}>
          Así de sencillo
        </h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 36 }} className="planes-pasos-grid">
          {PASOS.map(p => (
            <div key={p.numero} style={{ textAlign: 'center' }}>
              <div style={{ fontStyle: 'italic', fontWeight: 800, fontSize: 52, color: 'var(--coral)', lineHeight: 1 }}>
                {p.numero.replace('.', '')}
              </div>
